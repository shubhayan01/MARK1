...
const Homepage = () => {
  ...
  // Updated logout function that actually works
  const handleLogout = () => {
    if (window.confirm && window.confirm('Are you sure you want to logout?')) {
      ...
      // You can either:
      // 1. Redirect to login page (replace '/login' with your actual login route)
      window.location.href = '/login';
      
      // 2. Or reload the page to reset everything
      // window.location.reload();
      
      // 3. Or if using React Router, you'd use navigate('/login')
    }
  };
